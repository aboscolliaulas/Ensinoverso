...
const SettingsView: React.FC<SettingsViewProps> = ({ users, setUsers, bnccSkills, setBnccSkills, classes }) => {
  ...
  const handleApproveUser = async (user: AppUser) => {
    try {
      await dbService.save('users', user.id, { ...user, approved: true });
    } catch (err) { console.error(err); }
  };
                      ...
                      <div className="flex items-center gap-6">
                        ...
                        <div>
                          <div className="flex items-center gap-4">
                            ...
                            {!user.approved && (
                              <span className="bg-amber-500/20 text-amber-500 text-[8px] font-black px-3 py-1 rounded-full uppercase tracking-widest border border-amber-500/30 animate-pulse">Pendente</span>
                            )}
                          </div>
                    ...
                    <td className="py-8 text-right">
                      {!user.approved && (
                        <button 
                          ...
                          className="px-6 py-4 rounded-2xl bg-emerald-600 text-white font-black text-[9px] uppercase tracking-widest hover:bg-emerald-500 transition-all mr-3 shadow-lg"
                        >
                          Liberar Acesso
                        </button>
                      )}
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-6 bg-black/80 backdrop-blur-xl animate-fadeIn">
          <div className="w-full max-w-2xl bg-white text-gray-900 rounded-[4rem] p-16 space-y-10 shadow-2xl relative overflow-hidden">
            ...
            <div className="pt-6 space-y-4 relative z-10">
              <div className="flex gap-4">
                <button 
                  onClick={() => setEditingUser({...editingUser, approved: !editingUser.approved})} 
                  className={`flex-1 py-4 rounded-2xl font-black uppercase text-[9px] tracking-widest border transition-all ${editingUser.approved ? 'border-emerald-500 text-emerald-500 bg-emerald-50' : 'border-amber-500 text-amber-500 bg-amber-50'}`}
                >
                  Status: {editingUser.approved ? 'Aprovado' : 'Bloqueado'}
                </button>
              </div>
